...
#define CONFIG_BM_STORAGE_BACKEND_SD_MAX_RETRIES 8
//typedef long atomic_t;
/** Atomic value type (non-volatile, for parameters/returns) */
typedef int32_t atomic_val_t;

/** Atomic variable type (volatile, for shared memory) */
...
NRF_SDH_REQUEST_OBSERVER(sdh_state_req, 0) =
{
    .handler   = on_state_req_change,
    .context = NULL,
};
//NRF_SDH_STATE_EVT_OBSERVER(sdh_state_evt, on_state_evt_change, NULL, 0);
NRF_SDH_STATE_OBSERVER(sdh_state_evt,0) =
{
    .handler   = on_state_evt_change,
    .context = NULL,
};
...
static bool atomic_cas(atomic_t *target, atomic_val_t old_val, atomic_val_t new_val)
{
    //uint32_t nested;
    //(void) sd_nvic_critical_region_enter(&nested);  // 屏蔽用户中断
